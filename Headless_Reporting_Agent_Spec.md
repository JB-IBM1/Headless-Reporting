...
### Still Open
- **Dashboard hosting:** decide standalone web app vs. embed (affects state transport: URL params, server-side session, or both).
...
- **Dashboard real-data layer:** replace seeded mock data with session-state buckets written by the agent. Requires (a) `runQueryOverview()` in the backend to populate a journeys index in `SESSION_STORE`, and (b) refactoring the three dashboard screens to consume flat `NormalizedResponse` buckets instead of the mock journey-tree shape.